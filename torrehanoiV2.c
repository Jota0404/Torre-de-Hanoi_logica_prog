 ...
 */
void exibir_torres() {
    // Limpa a tela para uma melhor visualização (funciona em terminais Linux/macOS)
    // system("clear"); // Descomente para Linux/macOS
    // system("cls");   // Descomente para Windows

    // A largura máxima é determinada pelo maior disco para centralizar tudo
    int max_largura_disco = (num_discos * 2) - 1 + 2; // (underscores) + (parênteses)
    ...
    for (int i = num_discos - 1; i >= 0; i--) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            // Verifica se existe um disco no nível 'i' da torre 'j'
            if (i <= topo[j]) {
                int tamanho_disco = torres[j][i];
                int num_chars = (tamanho_disco * 2) - 1;
                int largura_visual_disco = num_chars + 2; // Adiciona os parênteses
                
                espacamento_lateral = (max_largura_disco - largura_visual_disco) / 2;

                // Imprime o espaçamento à esquerda para centralizar
                ...
                // Imprime o espaçamento à direita
                for(int k = 0; k < espacamento_lateral; k++) printf(" ");

            } else {
                // Se não houver disco, imprime o pilar vertical
    ...
    for (int j = 0; j < 3; j++) {
        for (int k = 0; k < max_largura_disco; k++) {
            printf("=");
        }
        printf("  ");
    }
    ...
    for (int j = 1; j <= 3; j++) {
        ...
        for (int k = 0; k < espacamento_lateral; k++) {
            printf(" ");
        }
        printf("%d", j);
        for (int k = 0; k < espacamento_lateral; k++) {
 ...
 */
int verificar_vitoria() {
    // A vitória ocorre quando a torre 3 (índice 2) está completa
